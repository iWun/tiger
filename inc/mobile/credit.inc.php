...
//	        }	 
        ...
        $xfcredit= pdo_fetchcolumn("SELECT SUM(num) FROM " . tablename($this->modulename.'_jl')." where type=0 and uid='{$fans['tkuid']}' and num<0  and  weid='{$_W['uniacid']}'");
        $czcredit= pdo_fetchcolumn("SELECT SUM(num) FROM " . tablename($this->modulename.'_jl')." where type=0 and uid='{$fans['tkuid']}' and num>0  and  weid='{$_W['uniacid']}'");
		...
		if($_W['isajax']){
            ...
            $list = pdo_fetchall("select * from ".tablename($this->modulename."_jl")." where weid='{$_W['uniacid']}' and type=0 and uid='{$uid}' order by id desc LIMIT " . ($pindex - 1) * $psize . ",{$psize}");
            $total = pdo_fetchcolumn("SELECT COUNT(*) FROM " . tablename($this->modulename.'_jl')." where type=0 and uid='{$uid}'  and  weid='{$_W['uniacid']}'");
            $list1=array();
            foreach($list as $k=>$v){
        ...
        include $this -> template('user/credit');