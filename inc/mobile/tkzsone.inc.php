...
global $_W, $_GPC;
	...
	//file_put_contents(IA_ROOT."/addons/tiger_newhu/inc/mobile/tkzs--ordernews.txt","\n:".$content,FILE_APPEND);
	$userInfo = @json_decode($content, true);
	$list=$userInfo['data'];
	//file_put_contents(IA_ROOT."/addons/tiger_newhu/inc/mobile/tkzs--ordernews.txt","\n:2".json_encode($list),FILE_APPEND);
	if(!empty($list)){
		foreach($list as $k=>$v){
			$fztype=pdo_fetch("select * from ".tablename($this->modulename."_fztype")." where weid='{$_W['uniacid']}' and tkzstype='{$v['goods_cate_id']}'");
			if($v['is_tmall']==1){
				$shoptype="B";
			}else{
				$shoptype="C";
			...
			$item = array(
						...
						'itemdesc'=>$v['goods_intro'],//推荐内容
						'itempic'=>$v['goods_pic'],//主图地址
						'itemprice'=>$v['goods_price'],//'商品原价', 
						'itemendprice'=>$v['goods_price']-$v['coupon_amount'],//商品价格,券后价
						'itemsale'=>$v['goods_sale_num'],//月销售
						'tkrates'=>$v['commission_rate'],//通用佣金比例
						'couponreceive'=>"",//优惠券总量已领取数量
						'couponsurplus'=>"",//优惠券剩余
						'couponmoney'=>$v['coupon_amount'],//优惠券面额
						'couponendtime'=>strtotime($v['coupon_end_time']),//优惠券结束
						'couponurl'=>"",//优惠券链接
						'shoptype'=>$shoptype,//'0不是  1是天猫',
						'quan_id'=>$v['coupon_id'],//'优惠券ID',  
    ...
    echo "淘客助手 第".$_GPC['page']."页采集【入库】成功";