<?php
 global $_W, $_GPC;
        ...
        if($op=='qb'){//全部
            ...
            $order = pdo_fetchall("select * from ".tablename($this->modulename."_order")." where weid='{$_W['uniacid']}' {$ddorwehre} and openid='{$fans['openid']}' order by id desc");
//             echo "<pre>";
//             print_r($order);
//             exit;
            foreach($order as $k=>$v){
                ...
                $tkorder = pdo_fetch("select * from ".tablename($this->modulename."_tkorder")." where weid='{$_W['uniacid']}' and orderid='{$v['orderid']}'  order by id desc");
                $tkordersum = pdo_fetchcolumn("select sum(xgyg) from ".tablename("tiger_newhu_tkorder")." where weid='{$_W['uniacid']}' and orderid='{$v['orderid']}' and orderzt!='订单失效'");
                                       ...
                                       if($v['type']==0){//自购
                                         ...
                                         if(!empty($credit1_zg)){
                                             ...
                                             //$zgdd = pdo_fetch("select * from ".tablename("mc_credits_record")." where uniacid='{$_W['uniacid']}' and uid='{$uid}' and remark='自购订单返积分:{$v['orderid']}'"); 
												 $result =pdo_update ( $this->modulename . "_order", array('sh'=>2,'yongjin'=>$tkorder['xgyg']), array ('id' => $v['id']));
												 if(!empty($result)){
