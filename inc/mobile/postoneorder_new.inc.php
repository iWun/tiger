...
// 			 				'tgwtitle'=>$news["adzone_name"],//推广位名称
						...
						if(!empty($news["adzone_id"])){
			 			...
			 			$ord=pdo_fetch ( 'select orderid,orderzt from ' . tablename ( $this->modulename . "_tkorder" ) . " where weid='{$_W['uniacid']}' and numid='{$news['item_id']}'  and (orderid='{$news['trade_id']}' or orderid='{$news['trade_parent_id']}')" );
						//print_r($ord);
						//if($ord){
							//echo $news['trade_id']."---";
