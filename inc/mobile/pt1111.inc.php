...
global $_W, $_GPC;
       ...
       $weid=$_W['uniacid'];
		...
		if ($_W['isajax']){
			        ...
			        if(empty($memberid)){
			        	...
			        	$tksign = pdo_fetch("SELECT * FROM " . tablename($this->modulename."_tksign") . " WHERE  memberid='{$memberid}'");
							...
							foreach($goods['result_list']['map_data'] as $k=>$v){
										//$goods=$v;
										if(!empty($v['coupon_amount'])){
											$conmany=$v['coupon_amount'];    
										}else{
											$conmany=0;
										}
																	
										 $itemendprice=$v['zk_final_price']-$conmany;   		
									 $status=1;
											 $list[$k]['itemtitle']=$v['title'];  //标题
											 $list[$k]['shoptype']=$v['user_type'];  //1天猫
											 $list[$k]['itemid']=$v['item_id'];//商品ID
											 $list[$k]['itemprice']=$v['zk_final_price'];//原价
											 $list[$k]['itemendprice']=$itemendprice;//折扣价-优惠券金额
											 $list[$k]['couponmoney']=$conmany;//优惠券金额
											 $list[$k]['itemsale']=$v['volume'];//月销量
											 $list[$k]['click_url']=$v['click_url'];//商品链接转链过的
											 $list[$k]['coupon_click_url']=$v['coupon_click_url'];//商品链接转链过的
											 $list[$k]['shopTitle']=$v['shop_title'];//店铺名称                 
					...
					//			                 $ratea=$this->sharejl($itemendprice,$goods['commission_rate']/100,$bl,$share,$cfg);
											 ...
											 $list[$k]['rate']=$ratea;
											 $list[$k]['couponnum']=$v['coupon_remain_count'];//剩余优惠券数量//用的是总量     
								}
								if(empty($list)){
									$pages=0;
								}else{
       ...
       include $this->template ( 'qtz/pt1111' );
?>
