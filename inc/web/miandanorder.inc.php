				...
				if($op=="pljs"){//更新订单状态
							if(!$_GPC['id']){
								...
								foreach ($_GPC['id'] as $id){
										$row = pdo_fetch("SELECT * FROM " . tablename($this->modulename.'_order') . " WHERE id = :id", array(':id' => $id));
...
// 				//                  
										...
										$tkrow = pdo_fetch("SELECT id,orderid,orderzt FROM " . tablename($this->modulename.'_tkorder') . " WHERE orderid = :orderid", array(':orderid' => $row['orderid']));
// 																		echo "<pre>";
// 																		print_r($tkrow);		
																			//	exit;
										 if($tkrow['orderzt']=="订单结算"){													 
											 $aupres=pdo_update($this->modulename."_order",array('sh'=>2), array('id' => $id));
											 if($aupres){
											 	  if($row['jltype']==1){//余额
													   $credit2_zg=$row['jl'];
														 if($row['type']==0){//自购
...
$page=$_GPC['page'];
 ...
 $cfg = $this->module['config'];
		...
		$psize = 20;
		$list1 = pdo_fetchall("select * from ".tablename($this->modulename."_mdorder")." where weid='{$_W['uniacid']}' {$where} {$whereuid} order by id desc LIMIT " . ($pindex - 1) * $psize . ",{$psize}");
		$total = pdo_fetchcolumn("SELECT COUNT(*) FROM " . tablename($this->modulename.'_mdorder')." where weid='{$_W['uniacid']}' {$where} {$whereuid}");
		$pager = pagination($total, $pindex, $psize);
		
		foreach($list1 as $k=>$v){
			$list[$k]['id']=$v['id'];
			$tkorder = pdo_fetch("select * from ".tablename("tiger_newhu_tkorder")." where weid='{$_W['uniacid']}' and orderid='{$v['orderid']}'");
			$list[$k]['orderzt']=$tkorder['orderzt'];
			...
			$list[$k]['yongjin']=$v['yongjin'];
			$list[$k]['type']=$v['type'];
			$list[$k]['sh']=$v['sh'];
			$list[$k]['msg']=$v['msg'];
			$list[$k]['itemid']=$v['itemid'];
			$list[$k]['jl']=$v['jl'];
			$list[$k]['jltype']=$v['jltype'];
        ...
        include $this->template ( 'miandanorder' );