...
global $_W;
		...
		$qtzlist = pdo_fetchall("select * from ".tablename("tiger_newhu_qtzlist")." where weid='{$_W['uniacid']}'  order by id desc");
		// echo "<pre>";
		// print_r($qtzlist);
		// exit;
		...
		if(checksubmit('submityc')){//隐藏
		        ...
		        foreach ($_GPC['id'] as $id){
					...
					if (empty($row)){
		             ...
		             pdo_update('tiger_newhu_appdh',array('showtype'=>1), array('id' => $id));
		        }
		...
		if(checksubmit('submitxs')){//显示
		        ...
		        foreach ($_GPC['id'] as $id){
					...
					if (empty($row)){
		             ...
		             pdo_update('tiger_newhu_appdh',array('showtype'=>0), array('id' => $id));
		        }
		        message('批量隐藏成功', referer(), 'success');        	
		}
